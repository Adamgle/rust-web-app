...
CREATE TABLE stocks (
    ...
    -- Absolute percent change since the last closing time.
    percent_change REAL NOT NULL CHECK (
        percent_change > -100
        AND percent_change < 100
    ),
    -- Represents the absolute change since the last closing time.
...
CREATE TABLE stocks_history (
    ...
    -- This enforces uniqueness of the pair, not one of the field alone. 
...
-- FROM
--     user_stocks
--     JOIN stocks ON stocks.id = user_stocks.stock_Sid
